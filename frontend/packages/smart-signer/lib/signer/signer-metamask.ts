...
export class SignerMetaMask extends Signer {
  ...
  async signTransaction({ transaction, requiredKeyType }: SignTransaction): Promise<string> {
    try {
      ...
      await verifyAuthorityOrThrow(authTx.toApiJson(), TTransactionPackType.HF_26, requiredKeyType ?? this.keyType, 'MetaMask');
      ...
      logger.info('MetaMask signTransaction produced %d signature(s)', authTx.transaction.signatures.length);
