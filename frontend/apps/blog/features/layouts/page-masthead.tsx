...
const PageMasthead: FC<PageMastheadProps> = ({
  ...
  <header className="relative mb-7 overflow-hidden rounded-panel border border-line-warn-3 accent-rail bg-[radial-gradient(125%_130%_at_0%_0%,rgb(var(--masthead-1))_0%,rgb(var(--masthead-2))_30%,rgb(var(--masthead-3))_58%,rgb(var(--masthead-4))_85%)] px-7 pb-5 pt-7">
    ...
    {eyebrow ? (
      ...
      <p className="relative z-10 mb-1.5 text-label font-medium font-ui uppercase tracking-label text-ink-brand-6/70 dark:text-ink-brand-6">
        {eyebrow}
      </p>
    ) : null}

    <Heading className="relative z-10 font-ui text-[34px] font-medium leading-[38px] tracking-[-0.015em] text-ink-2">
      {title}
    </Heading>
