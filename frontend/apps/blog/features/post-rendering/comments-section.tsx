...
// ★ 2026-09-15: the empty state below no longer mounts its own composer (it
// did from 2026-08-17 to 2026-09-15); the post's Reply opens the one composer
// directly above this section, so nothing here duplicates it.
    ...
    <div ref={sectionRef} className={commentsSectionClasses}>
      {replyCount === 0 ? (
        <div className="flex flex-col items-center gap-1.5 py-12 text-center" data-testid="comments-empty">
          ...
          {/* ★ NO SECOND COMPOSER HERE (owner, 2026-09-15: "the comment window
              already opens at bottom of post. so when i click the bottom one it
              opens 2 which looks horrible"). The post's own Reply opens the one
              composer directly above this section; the extra Reply trigger and
              the second ReplyTextbox this empty state mounted are gone. */}
        </div>
      ) : (
        <>
