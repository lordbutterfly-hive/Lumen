...
export default function ProposalStatsColumn({ vm }: { vm: ProposalViewModel }) {
  ...
  if (proposal.status === 'expired') {
    return (
      <div className="flex flex-col items-end gap-1 font-sans text-[13px] text-[#6b7280]" data-testid="proposal-stats-expired">
  ...
  if (proposal.status === 'inactive' && parseChainDate(proposal.start_date).isAfter(dayjs())) {
    return (
      <div className="flex flex-col items-end gap-1 font-sans text-[13px] text-[#6b7280]" data-testid="proposal-stats-upcoming">
  ...
  if (!isFunded) {
    return (
      <div className="flex flex-col items-end gap-1 font-sans text-[13px] text-[#6b7280]" data-testid="proposal-stats-unfunded">
        <div>
  ...
  return (
    <div className="flex flex-col items-end gap-1 font-sans text-[13px] text-[#6b7280]" data-testid="proposal-stats-funded">
      <div>
