...
/**
 * Read-only copy of features/wallet/components/estimated-value-strip.tsx for
 * the public wallet page. Same figures, same price feed, same dash-not-zero
 * rule. What changed: the description reads in the third person (the private
 * key says "in your wallet") and the testid carries the public prefix. Kept
 * as its own file so the private strip and its copy are never edited by
 * accident.
 */

import { useTranslation } from '@/blog/i18n/client';
import { useHiveMarketPrices } from '@/blog/features/wallet/hooks/use-hive-market-prices';
import { formatUsd } from '@/blog/features/wallet/lib/format-amount';
 ...
 * rail's price cards. Renders "a dash" instead of a number while prices are
 * loading/unavailable rather than guessing.
 */
export default function PublicEstimatedValueStrip({ figures }: { figures: WalletFigures }) {
  ...
  return (
    <div
      ...
      data-testid="public-estimated-value"
    >
      <div>
        <div className="text-[16px] leading-[24px] font-semibold text-ink-2">{t('wallet.estimated_value.title')}</div>
        <div className="mt-0.5 text-[14px] leading-[22px] text-ink-10">{t('wallet.public.estimated_value_description')}</div>
      </div>
