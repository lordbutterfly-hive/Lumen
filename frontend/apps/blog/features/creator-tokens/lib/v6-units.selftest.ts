...
check('wire form: a whole amount is a bare integer the LIVE v5.1 parser accepts ("2"), a fraction has two places', formatTokenAmount(2) === '2' && formatTokenAmount(1.5) === '1.50' && formatTokenAmount(0.01) === '0.01' && formatTokenAmount(42.64) === '42.64');
check('display form matches the contract print (always two places)', formatTokenAmountFixed(2) === '2.00' && formatTokenAmountFixed(1.5) === '1.50');
...
check('buy 1.5 -> "1.50", sell 0.25 -> "0.25", transfer 2 -> "2" (the pre-v6 shape), ask cap 22.49 -> "22.49"', buyPayload('hive:c', 1.5).tokens === '1.50' && sellPayload('hive:c', 0.25).tokens === '0.25' && transferTokensPayload('hive:c', 'hive:d', 2).amount === '2' && askPayload('hive:c', 'h', 28800, 22.49, 0).maxCredits === '22.49');
