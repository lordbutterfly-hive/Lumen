...
const Unavailable: FC<{ children: React.ReactNode }> = ({ children }) => (
  <div className="rounded-[14px] border border-dashed border-[#e4e6e9] px-5 py-6 text-center text-[13.5px] leading-[1.6] text-[#9ca3af]">{children}</div>
);
...
const HoldingRow: FC<{ h: HolderPosition }> = ({ h }) => (
  <div className="flex flex-wrap items-center gap-4 rounded-[16px] border border-[#ebebeb] bg-white px-5 py-4">
    ...
    <div className="min-w-0 flex-1">
      <Link href={`/creators/${h.creator}`} className="text-[15px] font-bold text-[#161511] hover:text-[#c0392b]">
        @{h.creator}
      </Link>
      {/* No bio line: it is not contract state, and the Hive profile is not read on this route. */}
    </div>
    <div className="text-right tabular-nums">
      <div className="text-[15px] font-bold text-[#161511]">{tok(h.tokensHeld)} tokens</div>
...
const RateStrip: FC<{ onRate: (score: number) => Promise<void>; busy: boolean }> = ({ onRate, busy }) => {
  ...
  return (
    <div className="mt-3 border-t border-[#f1f3f5] pt-3">
      <div className="mb-2 text-[12.5px] text-[#6b7280]">How did it go? Your rating is the creator’s public record.</div>
...
const AskCard: FC<{ a: Ask; onReclaim: () => Promise<void>; onRate: (score: number) => Promise<void>; busy: boolean; rating: boolean }> = ({
  ...
  return (
    <div className={`rounded-[16px] border bg-white px-5 py-4 ${reclaimable ? 'border-[#f6e2c4] bg-[#fdf6ec]' : 'border-[#ebebeb]'}`}>
      ...
      {a.contentHash ? <div className="mt-1 font-mono text-[11.5px] text-[#9ca3af]">ref {a.contentHash}</div> : null}
      {a.status === 'answered' ? <RateStrip onRate={onRate} busy={rating} /> : null}
      {reclaimable ? (
        <div className="mt-3 flex items-center justify-between gap-3">
          <div className="text-[13px] text-[#8a5a20]">You get {a.tokensEscrowed.toFixed(2)} tokens back to your balance — in full.</div>
          <button
            onClick={async () => {
              if (busy) return;
              setFailure(null);
            ...
            disabled={busy}
            className="rounded-[10px] bg-[#b45309] px-4 py-2 text-[13px] font-semibold text-white hover:bg-[#8a4207] disabled:opacity-50"
          >
        ...
        <>
          ...
          {reclaimable > 0 ? (
            <div className="mt-4 flex items-center justify-between gap-3 rounded-[12px] border border-[#f6e2c4] bg-[#fdf6ec] px-4 py-3">
              <span className="text-[13.5px] font-semibold text-[#b45309]">
