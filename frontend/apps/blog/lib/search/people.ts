 ...
 */
export const searchPeopleByPrefixCached = withTtlCache(loadPeopleByPrefix, (query: string) => query, {
  ttlMs: 60_000,
  max: 500,
...
export const searchPeopleByTopicCached = withTtlCache(loadPeopleByTopic, (query: string) => query, {
  ttlMs: 300_000,
  max: 500,
  shouldCache: (value) => Array.isArray(value)
});
...
async function loadPeopleByTopic(query: string): Promise<PersonResult[]> {
  const topic = normalizeSearchText(query);
  if (!topic) return [];
...
/**
 * `bridge.get_profile` for every name, in parallel (12 in 326ms measured on
 * api.openhive.network 2026-09-05). `allSettled` so one account whose profile
...
async function hydrateHiveProfiles(names: readonly string[]): Promise<PersonResult[]> {
  ...
  const settled = await Promise.allSettled(wanted.map((name) => getBridgeProfile(name)));
