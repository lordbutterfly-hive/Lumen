 ...
 */
function belongsOnLumen(sessionUser: User | undefined, targetIsLite: boolean): boolean {
  return sessionUser?.account_tier === 'lite' || targetIsLite;
}
...
async function actorFor(
  ...
  const check = await checkLiteActorById(actor.userId);
...
export async function followByName(
  ...
  if (!belongsOnLumen(sessionUser, target.isLite)) {
...
export async function followState(
  ...
  const lumenEdge = belongsOnLumen(sessionUser, target.isLite);
