...
// and the demo (see scenario.go, demo.go). Push events in the exact order
// they'd be emitted on-chain; Events replays them from a cursor exactly like
// a real paginated source would, including short reads under a limit — so
// tests exercising MockEventSource also exercise Index's poll-loop
// correctly against a source that doesn't just hand back everything at once.
//
...
// the Cursor doc in source.go); a real EventSource encodes it completely
// differently (see gql_source.go).
...
// Push appends one raw log line to the end of the source, in emission order.
// OutputID/BlockHeight/Seq are filled in automatically (OutputID = a
// synthetic "mock-N" id, Seq = the running index) so tests can just supply
// the JSON payload.
...
func (m *MockEventSource) Events(since Cursor, limit int) ([]RawEvent, Cursor, error) {
	...
	if end == start {
		...
		// edge case (every OTHER caught-up case already satisfies it,
		// since resuming from a non-empty cursor like "5" naturally
		// recomputes back to "5").
...
// Len reports how many events have been Pushed so far — a test/demo
// convenience, not part of EventSource.
