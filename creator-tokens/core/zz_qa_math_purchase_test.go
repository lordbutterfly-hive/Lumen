...
func qaMarketAtSupply(t *testing.T, S int64) (*MemStore, string) {
	...
	if S > 0 {
		if _, err := Buy(s, "hive:seed", c, 1000, qaBig(S)); err != nil {
			t.Fatalf("seed buy: %v", err)
		}
	}
	if got := getMoney(s, kSupply(c)); got.Cmp(qaBig(S)) != 0 {
...
func TestQAMath_Purchase_BudgetTable(t *testing.T) {
	...
	for _, S := range qaSupplies {
		for _, budget := range qaBudgets {
			...
			r, err := Buy(s, "hive:buyer", c, 2000, qaBig(n))
			if err != nil {
			...
			if r.Minted.Cmp(qaBig(n)) != 0 || BalanceOf(s, c, "hive:buyer").Cmp(qaBig(n)) != 0 {
			...
			if got := getMoney(s, kReserve(c)); got.Cmp(Area(qaBig(S+n))) != 0 {
				t.Fatalf("S=%d n=%d: reserve %s != area(%d) = %s", S, n, got, S+n, Area(qaBig(S+n)))
			}
...
func TestQAMath_Purchase_QuoteGrid(t *testing.T) {
	...
	for _, S := range qaSupplies {
		for _, n := range []int64{1, 2, 3, 5, 7, 9, 10, 11, 24, 25, 100} {
			cost, fee, total := qaTotalDue(S, n)
			t.Logf("%d\t%d\t%s\t%s\t%s\t%s", S, n, cost, fee, total, SpotRate(qaBig(S+n)))
...
func TestQAMath_Purchase_TokenIsWholeUnit(t *testing.T) {
	for _, bad := range []string{"0.5", "1.0", "1.000", "0.001", "1e3", "-1", " 1", "1 ", "", "0x10"} {
		if _, err := parseMoney(bad); err == nil {
			t.Fatalf("parseMoney(%q) accepted a non-integer token amount", bad)
		}
	}
	// "+1" is the whole unit 1 with a sign prefix; big.Int accepts it and nothing downstream can tell it apart.
	for _, good := range []string{"1", "01", "+1", "1000000000"} {
		if _, err := parseMoney(good); err != nil {
			t.Fatalf("parseMoney(%q) refused: %v", good, err)
		}
	}
	...
	// Minimum purchase is exactly one token at the current supply's step.
	cost, fee, total := qaTotalDue(0, 1)
	if cost.Cmp(qaBig(1007)) != 0 || fee.Cmp(qaBig(50)) != 0 || total.Cmp(qaBig(1057)) != 0 {
		t.Fatalf("first token = %s/%s/%s, want 1007/50/1057", cost, fee, total)
	}
	t.Logf("MIN_PURCHASE\tS=0\tcost=%s\tfee=%s\ttotal=%s", cost, fee, total)
}
...
func TestQAMath_Purchase_MainnetReplay(t *testing.T) {
	...
	if cost := BuyCost(qaBig(0), qaBig(1)); cost.Cmp(qaBig(1007)) != 0 {
	...
	if Area(qaBig(2)).Cmp(qaBig(2023)) != 0 {
		t.Fatalf("area(2) = %s, want 2023 (live hbd-temp reserve)", Area(qaBig(2)))
	}
	...
	if Area(qaBig(3)).Cmp(qaBig(3047)) != 0 {
		t.Fatalf("area(3) = %s, want 3047 (live blanchy reserve)", Area(qaBig(3)))
	}
}
...
func TestQAMath_Purchase_CurveRoundingIsPathIndependent(t *testing.T) {
	...
	for S := int64(0); S <= 2000; S += 7 {
		for _, n := range []int64{1, 2, 3, 13} {
			cost := BuyCost(qaBig(S), qaBig(n))
			back, err := SellProceeds(qaBig(S+n), qaBig(n))
			if err != nil {
				t.Fatal(err)
			}
			if back.Cmp(cost) != 0 {
			...
			sum := mZero()
			for i := int64(0); i < n; i++ {
				sum = mAdd(sum, BuyCost(qaBig(S+i), qaBig(1)))
			}
			if sum.Cmp(cost) != 0 {
