...
func TestGenEscrowFixtures(t *testing.T) {
	cases := []struct {
		...
		{"typical", escrowRec{
			...
			status: askPending, commissionCredits: big.NewInt(1200),
		...
		{"offering-zero", escrowRec{
			...
			status: askAnswered, commissionCredits: big.NewInt(0),
		...
		{"answerHash-contains-pipe", escrowRec{
			...
			status: askReclaimed, commissionCredits: big.NewInt(5),
