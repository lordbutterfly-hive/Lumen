...
### Token units (v6, 2026-09-22)

- **A token is divisible into `TokenScale` = 100 units (0.01 token).** State holds UNITS: `mb|`, `lots|`, `m|c|sup`, `m|c|cap`, escrow credits and commission, `em|`, `el|`. **The matured bucket is the exception, because magi-market reads it:** `bal|<holder>|<creator>` stays WHOLE tokens (little-endian u64, byte for byte the pre-v6 value) and the remainder below one token lives in `balf|<holder>|<creator>` (0..99 units, decimal); `MaturedOf` = bal x 100 + balf. `MinTradeUnits` = 1 (0.01), enforced at every entry point (`belowMinTrade`), `MinCap` = 100 units (1.00 token), `MaxCap` = 1e11 units.
- **The wire speaks tokens.** Every token amount a caller sends (`tokens`, `credits`, `maxCredits`, `firstBuy`, `amount`, `cap`, `newCap`) is a decimal token string with up to two places: `"2"` = 2.00 tokens = 200 units, `"1.5"`, `"0.01"`. `"1.005"`, `".5"`, `"1."`, exponents and whitespace are refused (`parseTokens`). Every token amount the contract returns or logs is `fmtTokens(units)` = `"1.50"`; events carry `v: 2`. **The marketplace door speaks WHOLE tokens, unchanged from v5.1:** `safeTransferFrom` and `approve` take an integer `amount` of whole tokens (a fraction can never cross the door; `TransferMatured` refuses any amount that is not a whole multiple), `balanceOf` and `allowance` answer whole tokens, allowances (`allow|`) are whole tokens and are not migrated, `tokenCreated.maxSupply` is whole tokens, and `TransferSingle.value` / `Approval.amount` are whole tokens: a movement emits `TransferSingle` only for the change in the account's WHOLE-token matured count (`emitTransferSingleWhole`), while the Lumen sibling `maturedMoved` always carries the exact decimal amount. Fractions stay fully spendable through Lumen's own rails (sell, transfer, refund, ask).
- **The curve is unchanged per whole token** and piecewise-linear inside a token: `Area(100 x S)` equals the old `Area(S)` for every S, so `R == Area(supply)` holds through the migration without touching a single reserve. `SpotRate(u)` is the rate of the token being filled, `ceil(u / 100)`. Fees floor to at least `MinFeeBaseUnits` = 1 base unit per trade, so splitting a sale into unit sales is never cheaper.
- **Asks settle in units:** `creditsForAsk = ceil(face x 100 / rate)`, so a 1.000 HBD service at a 1.015 HBD/token rate costs 0.99 tokens, not 1. Commission = `floor(units x 12%)`. The miss deterrent keeps `min(escrow, 100 units)`.
- **Lazy migration (`migrate_v6.go`).** The wasm wrapper reads and writes state through `WrapUnits`: the first touch of a v5.1 holder (`mb|`, `lots|`; `bal|` needs no scaling because it keeps its whole-token unit), market (`sup`, `cap`) or escrow (9 fields -> 10 with the `u2` marker) scales it by 100 exactly once and flags it (`u6|c|h`, `m|c|u6`), and only once every present value converted (a value that cannot be scaled leaves the flag unset so the next touch retries). Clocks, reserves, fees, TWAP rings and offerings are never touched. Proven on the mainnet snapshot `core/testdata/mainnet-v5-110125225.json` (`zz_v6_mainnet_fixture_test.go`).
- **`RefundPrice`** is still HBD per WHOLE token: `floor(reserve x 100 / supply-in-units)`.
