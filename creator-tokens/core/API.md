...
- **A token is divisible into `TokenScale` = 100 units (0.01 token).** State holds UNITS: `mb|`, `bal|` (matured, little-endian u64), `lots|`, `m|c|sup`, `m|c|cap`, escrow credits and commission, `em|`, `el|`. `MinTradeUnits` = 1 (0.01), `MinCap` = 100 units (1.00 token), `MaxCap` = 1e11 units.
- **The wire speaks tokens.** Every token amount a caller sends (`tokens`, `credits`, `maxCredits`, `firstBuy`, `amount`, `cap`, `newCap`) is a decimal token string with up to two places: `"2"` = 2.00 tokens = 200 units, `"1.5"`, `"0.01"`. `"1.005"`, `".5"`, `"1."`, exponents and whitespace are refused (`parseTokens`). Every token amount the contract returns or logs is `fmtTokens(units)` = `"1.50"`; events carry `v: 2`. `EvTransferSingle`/`EvApproved` (the magi-market door) stay raw integer units.
...
- **Lazy migration (`migrate_v6.go`).** The wasm wrapper reads and writes state through `WrapUnits`: the first touch of a v5.1 holder (`mb|`, `bal|`, `lots|`), market (`sup`, `cap`) or escrow (9 fields -> 10 with the `u2` marker) scales it by 100 exactly once and flags it (`u6|c|h`, `m|c|u6`). Clocks, reserves, fees, TWAP rings and offerings are never touched. Proven on the mainnet snapshot `core/testdata/mainnet-v5-110125225.json` (`zz_v6_mainnet_fixture_test.go`).
